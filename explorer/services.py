...
CLOVERLEAF_SINGLE_POSITIONS = SINGLE_POSITIONS + ['V1', 'V2', 'V3', 'V4', 'V5']
CLOVERLEAF_PAIRED_POSITIONS = PAIRED_POSITIONS + ['V11:V21', 'V12:V22', 'V13:V23', 'V14:V24', 'V15:V25', 'V16:V26', 'V17:V27']
CLOVERLEAF_SINGLE_FEATURES = {'A': 'A', 'C': 'C', 'G': 'G', 'U': 'U', 'absent': '-'}
CLOVERLEAF_PAIRED_FEATURES = {
...
LABELS.update(CLOVERLEAF_PAIRED_FEATURES)

TILEMAP_PAIRED_LABELS = {
...
def gather_cloverleaf_freqs(clade_txid, isotype):
  ...
  for freq in freqs_qs.values():
    ...
    if position in CLOVERLEAF_SINGLE_POSITIONS:
      freqs[position] = {feature: freq[feature] for feature in CLOVERLEAF_SINGLE_FEATURES}
    elif position in CLOVERLEAF_PAIRED_POSITIONS:
      freqs[position] = {CLOVERLEAF_PAIRED_FEATURES[pair]: freq[pair] for pair in CLOVERLEAF_PAIRED_FEATURES}
  return freqs
  ...
  for colname in cons:
    ...
    if position in CLOVERLEAF_SINGLE_POSITIONS:
      ...
      if cons[colname] == '': 
        ...
        position5_consensus, position3_consensus = TILEMAP_PAIRED_LABELS[cons[colname]]
