...
def process_tscan_output(genomes_df):
  ...
  for row in genomes_df.itertuples():
    ...
    sstofa3_path = os.path.join(os.path.dirname(__file__), 'sstofa3')
    subprocess.call('perl {} {} "" 1 0 > {}'.format(sstofa3_path, row.ss_file, row.tRNA_file), shell = True)
...
def get_iso_scores(iso_file):
  ...
  iso_scores['isoscore_ac'] = [iso_scores.at[idx, col] for idx, col in zip(iso_scores.index, iso_scores.iloc[:, 1])]
...
def annotate_trnas(trnas):
  ...
  trnas['primary'] = ~trnas.duplicated(subset=['species', 'isotype', 'score'], keep='first')
  message('done\n')
